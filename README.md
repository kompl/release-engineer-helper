...
make install          # + симлинк /usr/local/bin/bambai (спросит sudo)
```

После `make install` тула запускается из любой директории: разыменовав симлинк, она берёт `config.yaml` из каталога проекта. Пути внутри конфига при этом должны быть абсолютными.
...
Область анализа задаётся одним из трёх взаимоисключающих режимов:

```bash
./bambai --by-json                          # область из repo_branches.json (режим по умолчанию)
./bambai --by-log                           # область из лога, с фазой Parse
./bambai -p hydra-server                    # все ветки проекта из projects.json
./bambai -p hydra-server -b master -b v6.3  # только указанные ветки
```

- `--by-json` / `--by-log` аргументов не принимают: пути берутся из `input.repo_branches_file` и `input.log_file`.
- `-p` / `--project` — максимум один на запуск; имя проверяется по ключам `projects.json`, при опечатке запуск падает со списком доступных проектов.
- `-b` / `--branch` — флаг повторяемый, работает только вместе с `-p`. Указанные ветки собираются как есть; список веток в `projects.json` задаёт лишь набор по умолчанию, когда `-b` не указан.
- В режиме `-p` файл `repo_branches.json` не читается и не перезаписывается — разовый отчёт не оставляет следов в рабочем дереве.

Флаг `--stdout` печатает JSON-отчёт в stdout вместо файла — весь остальной вывод (прогресс-бары, заголовки фаз, ошибки) уходит в stderr, поэтому поток можно пайпить:

```bash
bambai -p hydra-server -b master --stdout | jq '.projects[].failed_tests'
```

Флаг перекрывает `output.generate_json`, файл в `output.dir` не создаётся, а при пустой области анализа в stdout уходит валидный пустой отчёт (`{"generated_at": ..., "projects": {}}`) — пайп не ломается. HTML-отчёты по-прежнему управляются ключом `output.generate_html`.

Путь к конфигу берётся из `--config`; без флага это `config.yaml` в текущей директории, а если его там нет — `config.yaml` рядом с бинарём (симлинк разыменовывается).
...
input:
  ...
  projects_file: "projects.json"           # Справочник всех проектов и их веток (для -p / -b)
  ignore_tasks:                            # Задачи-исключения (не парсить)
    - "ADM-3191"
    - "INT-570"
```

Фаза `Parse` — единственная опциональная, и выполняется она только в режиме `--by-log`. В режимах `--by-json` и `-p` область анализа берётся готовой. Остальные фазы (Collect → Analyze → Enrich → Render) образуют неразрывную цепочку и выполняются всегда.

`projects.json` — справочник вида `{repo: [branch, ...]}` (тот же формат, что и `repo_branches.json`), перечисляющий все доступные проекты и их ветки. Читается только в режиме `-p`: даёт список допустимых имён проектов и набор веток по умолчанию.
