...
- `additional` — исторические данные по классифицированным тестам, не упомянутым в `failed_tests`: пофикшенные (с `probable_fix` — вероятный чинящий коммит) и flaky/stable, не упавшие в последнем ране (у всех — `last_failed`: когда падал в последний раз)
...
3. **Flaky тесты** — оценить масштаб проблемы:
   ...
   - Смотреть не только `failed_tests` последнего рана, но и flaky из `additional` — они не упали в последнем ране, но продолжают шуметь в истории
